...
class TestSystemdServiceRefresh:
    ...
    def _isolate_linux_user_systemd_preflight(self, monkeypatch):
        """These unit tests mock systemctl and must not probe macOS D-Bus."""
...
class TestGatewaySystemServiceRouting:
    @pytest.fixture(autouse=True)
    def _isolate_linux_user_systemd_preflight(self, monkeypatch):
        """Routing tests provide their own systemd state and subprocess mocks."""
        monkeypatch.setattr(gateway_cli, "_preflight_user_systemd", lambda: None)
...
class TestProfileArg:
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        # restart kill its predecessor → self-kill loop. The systemd unit and
        # launchd plist must NOT use --replace; the supervisor owns the
        # lifecycle. (--replace stays on detached/manual fallback paths.)
