...
def test_restart_coordinator_allows_only_preflight_registry_transition(tmp_path):
    registry = load_runtime_registry(_registry_file(tmp_path))
    profile = registry.require("lumi")
    revision = hashlib.sha256((profile.home / "config.yaml").read_bytes()).hexdigest()
    driver = _Driver()
    ...
    def health(url, timeout):
        ...
        if driver.restart_calls == 0:
            payload["runtime_identity"]["registry_revision"] = "old-registry"
        return payload
    ...
    result = coordinator.restart("lumi", allow_registry_transition=True)
    assert result.old_pid == 101
    assert result.new_pid == 102
    assert driver.restart_calls == 1
...
def test_restart_coordinator_cli_forwards_release_transition_flag(monkeypatch, tmp_path):
    ...
    class FakeCoordinator:
        ...
        def restart(
            ...
            profile,
            *,
            allow_release_transition,
            allow_config_transition,
            allow_registry_transition,
        ):
            calls.update(
                profile=profile,
                allow_release_transition=allow_release_transition,
                allow_config_transition=allow_config_transition,
                allow_registry_transition=allow_registry_transition,
            )
    ...
    assert calls == {
        ...
        "allow_registry_transition": True,
    }
